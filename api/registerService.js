...
//const fullNameRegex = /^[A-ZÀ-Y][A-ZÀ-Y]
...
function sendErrorsOrNext(req, res, next) {  // aaa
...
function register(req, res, next) {
    ...
    const complement = req.body.complement || ''
        ...
        if(mail == null || mail == ""){
            return res.status(400).send({ alert: ["o campo E-mail é obrigatorio."]})

        }

        
        if(!mail.match(mailRegex)){
            return res.status(400).send({ alert: ["o e-mail informado é inválido. Informe um e-mail"]})
